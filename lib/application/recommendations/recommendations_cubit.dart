  ...
  void registerRecommendor(RegisteredRecommendor? recommendor) async {
    if (recommendor == null) {
      ...
      if (recommendor.email == null) {
        ...
        final failureOrSuccess = await recommendationsRepo.checkIfRecommendorAlreadyExists(email: recommendor.email!);
        failureOrSuccess.fold(
          (failure) => emit(RecommendorRegisterFailureState(failure: failure)),
          (emailExists) async {
            if (emailExists) {
              emit(RecommendorAlreadyExistsFailureState());
            } else {
              final failureOrSuccessRegister = await recommendationsRepo.registerRecommendor(recommendor: recommendor);
