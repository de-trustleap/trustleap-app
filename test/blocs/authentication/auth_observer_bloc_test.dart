...
void main() {
  ...
  group("AuthObserverBloc_AuthObserverStartedEvent", () { 
    ...
    test("should call auth repo if event is added", () async {
      // Given
      when(mockAuthRepo.observeAuthState()).thenAnswer((_) => Stream<User?>.fromIterable([testUser]));
      // When
    ...
    test("should emit AuthObserverStateUnAuthenticated when event is added and there is no user", () {
      // Given
      final expectedResult = [
        AuthObserverStateUnAuthenticated(),
      ];
      when(mockAuthRepo.observeAuthState()).thenAnswer((_) => Stream<User?>.fromIterable([null]));
      // Then
