...
void main() {
  group('PagebuilderWidgetFinder_findWidgetById', () {
    ...
    test('should return null if widget is not found by id', () {
      final nonExistentWidgetId = UniqueID.fromUniqueString("nonExistentWidget");
      final foundWidget = widgetFinder.findWidgetById(mockPageBuilderPage, nonExistentWidgetId);
      expect(foundWidget, isNull);
    });

    test('should find widget in children of a column widget', () {
      final foundWidget = widgetFinder.findWidgetById(mockPageBuilderPage, widgetId1);
      expect(foundWidget?.id, equals(widgetId1));
    });

    test('should find widget in a row widget children', () {
      final foundWidget = widgetFinder.findWidgetById(mockPageBuilderPage, widgetId3);
    ...
    test('should return null if widget is not found in containerChild or children', () {
      final nonExistentWidgetId = UniqueID.fromUniqueString("nonExistentWidget");
      final foundWidget = widgetFinder.findWidgetById(mockPageBuilderPage, nonExistentWidgetId);
      expect(foundWidget, isNull);
    });
